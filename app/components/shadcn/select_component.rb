class Shadcn::SelectComponent
  ...
  def call
    view_context.content_tag :select, @content, name: name, class: tw("h-full w-full border-none bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-0 disabled:cursor-not-allowed disabled:opacity-50", @options[:class])
  end
end
